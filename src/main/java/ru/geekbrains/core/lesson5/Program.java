...
public class Program {
    ...
    private static void writeFileContents(String fileName, int length) throws IOException {
        try(FileOutputStream fileOutputStream = new FileOutputStream(fileName)){
            fileOutputStream.write(generateSymbol(length).getBytes(StandardCharsets.UTF_8));
        }
    }
    ...
    private static void backup(File fileIn1, File fileIn2) throws IOException {
        if(!fileIn2.exists()) {
            fileIn2.mkdir();
        }
        ...
        for (int i = 0; i < files.length; i++) {
            if(files[i].isFile()) {
                String s = fileIn2.getCanonicalPath() + "/" + files[i].getName();
                System.out.println(files[i].getName());
                fileOutputInput(files[i].getName(), s);
            }
        }
    }
    private static void fileOutputInput(String fileIn1, String fileIn2) throws IOException {
        // На запись
        try(FileOutputStream fileOutputStream = new FileOutputStream(fileIn2)){
            int c;
            // На чтение
            try(FileInputStream fileInputStream = new FileInputStream(fileIn1)){
                while((c = fileInputStream.read()) != -1){
